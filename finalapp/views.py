...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('shop:login')  # Redirect to home page or wherever appropriate
    else:
...
def add_review(request, movie_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            review.movie = movie
            # Set the user as the authenticated user if available, otherwise set as a placeholder value
            if request.user.is_authenticated:
                review.user = request.user
            else:
                # Use a placeholder value for non-registered users
                non_user = User.objects.get_or_create(username='Non-user')[0]
                review.user = non_user
            review.save()
            return redirect('shop:movie_detail', movie_id=movie_id)
    else:
        form = ReviewForm()
...
def movie_detail(request, movie_id):
    ...
    return render(request, 'movie_detail_1.html', {'movie': movie, 'reviews': reviews})
